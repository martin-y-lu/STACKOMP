  ...
  for(int i=0;i<DATA.line_count;i++){
    ...
    for(int j=0; j<DATA.num_columns;j++){
      if(i==DATA.ID_POS.Y && j==DATA.ID_POS.X){
        ...
        sprintf(temp_buff,"%c",DATA.cell_dat[i][j].value);
        strcat(text_buffer,temp_buff);
      }
    }
    strcat(text_buffer,"\n");
  }
...
int main(){
    ...
    while(running){
      if(show_debug_info){
        sprintf(text_buffer,"Memory Value:");
        if(DATA.MEM_TYPE==CHAR){
          sprintf(temp_buff," CHAR %c  ",DATA.MEM_VAL);
        }else if(DATA.MEM_TYPE==NUM){
          sprintf(temp_buff," NUM %d   ",DATA.MEM_VAL);
        }else{
          sprintf(temp_buff," FREE %d   ",DATA.MEM_VAL);
        }
        strcat(text_buffer,temp_buff);
        switch(DATA.dir){
          case RIGHT:
            sprintf(temp_buff,"Direction:>  ");
            break;
          case DOWN:
            sprintf(temp_buff,"Direction:v  ");
            break;
          case LEFT:
            sprintf(temp_buff,"Direction:<  ");
            break;
        ...
        strcat(text_buffer,temp_buff);
        sprintf(temp_buff,"ID_POS.X: %d ID_POS.Y: %d \n",DATA.ID_POS.X,DATA.ID_POS.Y);
        strcat(text_buffer,temp_buff);
      }
      bool halt = interpret();
      if(halt){
